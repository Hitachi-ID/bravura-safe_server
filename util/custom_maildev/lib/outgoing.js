...
function relayMail (emailObject, emailStream, isAutoRelay, done) {
  ...
  recipients = handlePlusAddresses(recipients)
...
function handlePlusAddresses (emailAddrs) {
  for (let i = 0; i < emailAddrs.length; i++) {
    if (emailAddrs[i].indexOf('+') !== -1) {
      const element = emailAddrs[i]
      const first = element.split('+')[0]
      const second = element.split('@').pop()
      emailAddrs[i] = first + '@' + second
    }
  }
