...
function Uninstall() {
    ...
    if ($keepDatabase -eq "y") {
        ...
        $uninstallAction = $( Read-Host "Are you sure you want to uninstall Bravura Safe? (y/n)" )
    } else {
    ...
    if ($uninstallAction -eq "y") {
        Write-Host "uninstalling Bravura Safe..."
        Docker-Compose-Down
        Write-Host "Removing $outputDir"
        Remove-Item -Path $outputDir -Force -Recurse
        Write-Host "Bravura Safe uninstall complete!"
    } else {
        Write-Host "Bravura Safe uninstall canceled."
        Exit
    }

    Write-Host "(!) " -f red -nonewline
        $purgeAction = $( Read-Host "Would you like to purge all local Bravura Safe container images? (y/n)" )
