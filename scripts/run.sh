...
function install() {
    LETS_ENCRYPT="n"
    echo -e -n "${CYAN}(!)${NC} Enter the domain name for your Bravura Pass instance (ex. bp.example.com): "
    read DOMAIN
    echo ""
    ...
    docker run -it --rm --name setup -v $OUTPUT_DIR:/bitwarden \
        --env-file $ENV_DIR/uid.env $REPO/setup:$COREVERSION \
...
function updateDatabase() {
    ...
    docker run -i --rm --name setup \
        -v $OUTPUT_DIR:/bitwarden --env-file $ENV_DIR/uid.env $REPO/setup:$COREVERSION \
...
function update() {
    ...
    docker run -i --rm --name setup -v $OUTPUT_DIR:/bitwarden \
        --env-file $ENV_DIR/uid.env bravura_vault/setup:$COREVERSION \
...
function uninstall() {
    ...
    then
        ...
        echo -e -n "${RED}(SAVED DATABASE FILES: YES): WARNING: ALL DATA WILL BE REMOVED, INCLUDING THE FOLDER $OUTPUT_DIR): Are you sure you want to uninstall Bravura Safe? (y/n): ${NC}"
        read UNINSTALL_ACTION
    else
        echo -e -n "${RED}WARNING: ALL DATA WILL BE REMOVED, INCLUDING THE FOLDER $OUTPUT_DIR): Are you sure you want to uninstall Bravura Safe? (y/n): ${NC}"
        read UNINSTALL_ACTION
    fi
    ...
    then
        echo "Uninstalling Bravura Safe..."
        dockerComposeDown
    ...
    echo -e -n "${RED}(!) Would you like to purge all local Bravura Safe container images? (y/n): ${NC}"
    read PURGE_ACTION
    if [ "$PURGE_ACTION" == "y" ]
    then
        dockerPrune
        echo -e -n "${CYAN}Bravura Safe uninstall complete! ${NC}"
    fi
